...
## الدفع بالبطاقات ومسار الاستثناء اليدوي

الإعداد الإنتاجي يستخدم `PAYMENT_PROVIDER=xpay` لدفع جميع العملاء بالبطاقات
البنكية عبر صفحة XPay المستضافة. بعد نجاح الدفع ينتقل العميل إلى واتساب لإرسال
المتطلبات وتنفيذ الخدمة.

يبقى `PAYMENT_PROVIDER=manual` متاحًا كمسار استثناء إداري للطلبات التي يتم
ترتيب تحصيلها خارج الموقع عبر واتساب. يُنشأ الطلب بحالة `pending_payment` دون
سجل دفع وهمي، ولا يصبح مدفوعًا إلا عبر `POST /api/v1/admin/payments/manual`.
يتطلب المسار صلاحية مدير، ومبلغًا مطابقًا لإجمالي الطلب، ويسجل طريقة التحصيل
والمرجع والتاريخ وسجل تدقيق دائم.
