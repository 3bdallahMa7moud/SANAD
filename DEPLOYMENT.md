...
لاستخدام Gmail SMTP، اضبط `SMTP_HOST=smtp.gmail.com` و`SMTP_PORT=465` و`SMTP_SECURE=true`، ثم ضع عنوان حساب Google الكامل في `SMTP_USER` و**كلمة مرور تطبيق Google** في `SMTP_PASSWORD` (وليست كلمة مرور الحساب). يتطلب إنشاء كلمة مرور التطبيق تفعيل التحقق بخطوتين في الحساب. يرسل التطبيق افتراضيًا من `SMTP_USER`؛ استخدم `SMTP_FROM_EMAIL` فقط إذا كان العنوان مفعّلًا كاسم إرسال بديل في Gmail. اترك `RESEND_API_KEY` فارغًا عند اختيار SMTP. بعد حفظ البيانات، أعد إنشاء حاوية `api` وأرسل رسالة اختبار إلى صندوق تملكه للتحقق من التسليم.
